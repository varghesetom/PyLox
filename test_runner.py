...
def python_interpreter(name, tests):
  INTERPRETERS[name] = Interpreter(name, 'python', ['python', 'Lox.py'], tests) 
  PYTHON_SUITES.append(name)

python_interpreter('jlox', {
  'test': 'pass',
...
python_interpreter('chap04_scanning', {
  # No interpreter yet.
  'test': 'skip',
  'test/scanning': 'pass'
})
...
python_interpreter('chap06_parsing', {
  # No real interpreter yet.
  'test': 'skip',
  'test/expressions/parse.lox': 'pass'
})

python_interpreter('chap07_evaluating', {
  # No real interpreter yet.
  'test': 'skip',
  'test/expressions/evaluate.lox': 'pass'
})

python_interpreter('chap08_statements', {
  'test': 'pass',
...
python_interpreter('chap09_control', {
  'test': 'pass',
...
python_interpreter('chap10_functions', {
  'test': 'pass',
...
python_interpreter('chap11_resolving', {
  'test': 'pass',
...
python_interpreter('chap12_classes', {
  'test': 'pass',
...
python_interpreter('chap13_inheritance', {
  'test': 'pass',
...
#    self.validate_exit_code(exit_code, error_lines)
  ...
  def validate_runtime_error(self, error_lines):
    if len(error_lines) < 2:
      self.fail('Expected runtime error "{0}" and got none.', self.runtime_error_message) 
      return
...
def run_script(path):   
  '''
  Instantiating a Test for each file we see in our test directory" 
  '''
